...
import { TrendingUp, Award, Target, Users, Droplets, Leaf, Voicemail as Soil, Bug } from 'lucide-react';
import { useUser } from '../context/UserContext';
import ProgressCard from './ProgressCard';
import BadgeDisplay from './BadgeDisplay';

const Dashboard: React.FC = () => {
  const { profile, missions, badges } = useUser();
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-green-100">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-green-600">{profile.sustainabilityScore}</p>
            </div>
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-green-100">
          ...
          <div className="mt-3">
            ...
            <p className="text-xs text-gray-500 mt-1">
              {1000 - (profile.totalPoints % 1000)} points to level {profile.level + 1}
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-green-100">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <span className="font-semibold text-green-600">{completedMissions.length}/{missions.length}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-gray-600">Badges Earned</span>
              <span className="font-semibold text-green-600">{earnedBadges.length}/{badges.length}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-gray-600">Total Points</span>
              <span className="font-semibold text-green-600">{profile.totalPoints.toLocaleString()}</span>
            </div>
          </div>
        </div>
      </div>
